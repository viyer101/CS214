...
compare.c - This code analyzes a set of input files and computes the Jensen-Shannon Distance between every pair based on their word frequency distributions. This program supports directories and files, it traverses through the files recursively and includes only files that are a matching pair (I.E. both files must be txt, you can't have different file structures).
